...
- `createWidgetRegistry()` — widget definitions: `sensor-value`,
  `switch`, `history-chart`, `room-device-list`; each with
  category, `defaultSize`, `supportedSizes` and `suggestForCapabilities`
  (filters by the selected device's capabilities). The `connection` widget
  was retired (Phase 1) and is no longer registered.
...
- `ui/widgets/` — the four built-ins. `HistoryChartWidget` queries **its own
